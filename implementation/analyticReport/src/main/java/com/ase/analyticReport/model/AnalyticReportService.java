package com.ase.analyticReport.model;

import com.ase.analyticReport.model.data.AnalyticReportEvent;
...
public class AnalyticReportService {
    ...
    public ResponseEntity<AnalyticReportEvent> getAnalyticReportEvent(String eventID)
        ...
        LOGGER.debug(
                "getAnalyticReportOrganizer: Creating analytic report from eventID {}, event fetched with eventService",
                eventID);
    ...
    public ResponseEntity<AnalyticReportFeedback> getAnalyticReportFeedback(String eventID) {
        ...
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<FeedbackList> feedbackList = restTemplate.getForEntity(
                GET_FEEDBACK_ENDPOINT + eventID, FeedbackList.class);
        LOGGER.debug(
                "getAnalyticReportAttendee: Creating analytic report from eventID {}, feedbacks fetched with feedbackService",
                eventID);
        return new ResponseEntity<>(new AnalyticReportFeedback(eventID, feedbackList.getBody()),
                HttpStatus.OK);
    }
